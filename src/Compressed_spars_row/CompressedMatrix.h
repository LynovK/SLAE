...
public:
    CompressedMatrix<T>(std::vector<element<T>> matrix_, unsigned int n, unsigned int m) noexcept{ // matrix приходит уже отсортированная
    ...
    std::vector<T> dot(const std::vector<T> &X) const noexcept{
        ...
        for(auto i = 0; i < counts_vector_.size() - 1; ++i){
            unsigned int k = counts_vector_[i + 1] - counts_vector_[i];
            unsigned int b = counts_vector_[i];
            x.push_back(0);
            for(auto j = 0; j < k; ++j){
                x[i] += vector_of_values_[b + j]*X[colomn_index_vector_[b + j]];
            }
        }
        return x;
    }
